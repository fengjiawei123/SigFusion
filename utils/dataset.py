...
class H5Dataset_Fusion(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        IR = np.array(h5f['ir_patchs'][key])
        VIS = np.array(h5f['vis_patchs'][key])
        fusion = np.array(h5f['fusion_patchs'][key])
        h5f.close()
        return torch.Tensor(VIS), torch.Tensor(IR), torch.Tensor(fusion)

class H5Dataset_NAT(Data.Dataset):
    def __init__(self, h5file_path):
        self.h5file_path = h5file_path
        h5f = h5py.File(h5file_path, 'r')
        self.keys = list(h5f['NAT_patchs'].keys())
        h5f.close()
    ...
    def __getitem__(self, index):
        ...
        key = self.keys[index]
        # IR = np.array(h5f['ir_patchs'][key])
        NAT = np.array(h5f['NAT_patchs'][key])
        # VIS = np.array(h5f['vis_patchs'][key])
        h5f.close()
        # return torch.Tensor(VIS), torch.Tensor(IR)
        return torch.Tensor(NAT)