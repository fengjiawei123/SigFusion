    ...
    for parent, dirnames, filenames in os.walk(file_name):
        for filename in filenames:
            if filename.lower().endswith(
                    ('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tif', '.npy')):
...
train_num = 0
for fusion_name in ['train_GT']:

    IR_files = sorted(get_img_file(r"Datasets/MEF/SICE/train/train_OVER/"))
    VIS_files = sorted(get_img_file(r"Datasets/MEF/SICE/train/train_UNDER/"))
    fusion_files = sorted(get_img_file(r"Datasets/MEF/SICE/train/" + fusion_name))
