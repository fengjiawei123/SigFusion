...
train_num = 0
for Modality in ['MIF', 'MFIF', 'VIF','MEF']:

    IR_files = sorted(get_img_file(r"results/PhaseⅢ_TEST/"+ Modality +"/SourceA/"))
    VIS_files = sorted(get_img_file(r"results/PhaseⅢ_TEST/"+ Modality +"/SourceB/"))
    ...
    for i in tqdm(range(len(IR_files))):
            ...
            I_IR = imread(IR_files[i]).astype(np.float32).transpose(2,0,1)/255.   # [1, H, W] Float32
            I_IR = rgb2y(I_IR) 
