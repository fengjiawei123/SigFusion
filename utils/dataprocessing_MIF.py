...
data_name="Medical_train_fusion"
img_size=256   #patch size
stride=400     #patch stride
...
train_num = 0
for fusion_name in ['EMFusion','U2fusion','MetaFusion', 'DIDFuse', 'CDDFuse', 'FusionGAN', 'GAMMcC', 'LRRNet','EMMA' , 'MUFusion', 'SDNet', 'MURF']:

    IR_files = sorted(get_img_file(r"data\Medical\medical\CT_MRI\CT"))
    VIS_files = sorted(get_img_file(r"data\Medical\medical\CT_MRI\MRI"))
    fusion_files = sorted(get_img_file(r"data\Medical/" + fusion_name))
    ...
    for i in tqdm(range(len(IR_files))):
            I_VIS = imread(VIS_files[i]).astype(np.float32) / 255.  # 先读取并转换为浮点型
            if I_VIS.ndim == 3:  # 如果是RGB图像
                I_VIS = I_VIS.transpose(2, 0, 1)  # [3, H, W] Uint8->float32
                I_VIS = rgb2y(I_VIS)
            elif I_VIS.ndim == 2:  # 如果是灰度图像
                I_VIS = I_VIS[None, :, :] # [1, H, W] Float32

            I_IR = imread(IR_files[i]).astype(np.float32) / 255.  # 先读取并转换为浮点型
            if I_IR.ndim == 3:  # 如果是RGB图像
                I_IR = I_IR.transpose(2, 0, 1)  # [3, H, W] Uint8->float32
                I_IR = rgb2y(I_IR)
            elif I_IR.ndim == 2:  # 如果是灰度图像
                I_IR = I_IR[None, :, :] # [1, H, W] Float32
            # 处理fusion图像
            ...
            for ii in range(I_IR_Patch_Group.shape[-1]):
                ...
                if not (bad_IR or bad_VIS or bad_fusion):
