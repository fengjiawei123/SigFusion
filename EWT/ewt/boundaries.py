...
#     n = 3
#
#     # 进一步调整 num_iter 以减少内存使用
#     num_iter = int(max(np.ceil(len(f) / (n * 20)), 3))  # 调整因子20
#
#     # 减少输入数据的尺寸
#     if len(f) > 100000:  # 如果数据非常大，分块处理
#         results = []
#         chunk_size = 100000
#         for i in range(0, len(f), chunk_size):
#             f_chunk = f[i:i + chunk_size]
#             results.append(GSS(f_chunk))
#         return np.concatenate(results, axis=0)
#
#     # 使用 float32 以减少内存使用
#     plane = np.zeros([len(f), num_iter + 1], dtype=np.float32)
#     plane[:, 0] = localmin(f)
#
#     # 定义尺度空间核（离散高斯核）
#     ker = np.exp(-t) * iv(np.arange(-n, n + 1), t)
