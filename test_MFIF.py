...
import torchvision
from utils.CustomDataset_color_double import MyDataset
import numpy as np
import cv2
'''
------------------------------------------------------------------------------
...
train_ir_dir = "Test/MFFW/source_1"
train_vi_dir = "Test/MFFW/source_2"

train_dataset = MyDataset(root_dir_mri=train_ir_dir, root_dir_ct=train_vi_dir)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False)

ckpt_path = 'checkpoints/MFIF.pth'
...
for i, (data_IR_y,data_IR_uv,data_VIS_y, data_VIS_uv,names) in enumerate(train_loader):
    ...
    with torch.no_grad():
        ...
        for b in range(batch_size):
            ...
            save_name = names[b]  # 原文件名（带后缀）
            save_path = os.path.join(path, save_name)
            torchvision.utils.save_image(data_Fuse_color, save_path)
